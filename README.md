...
To run a focused dev server for a single app or package, use `--filter`:

```sh
pnpm --filter web dev      # Next.js web app
pnpm --filter server dev   # Fastify API server
...
See [VERSIONING.md](./VERSIONING.md) for details on our versioning and release strategy.
