...
function isTypographyGroup(group: DTCGTokenGroup): boolean {
  ...
  // Must have at least one typography property token
  return Object.keys(group).some((key) => {
    ...
    return (
      ...
      typeof value === "object" &&
      "$type" in value &&
      typeof (value as DTCGToken).$type === "string"
    );
  });
}
...
export function flattenDTCGTokens(
  ...
  const processNode = (
    ...
    if ("$type" in node && typeof node.$type === "string") {
      // Skip individual typography property tokens - they'll be composed from groups
      // Check by path segment name, not just type (fontSize has type "dimension")
      const typographyPropertyNames = new Set([
        "fontFamily",
        "fontSize",
        "fontWeight",
      ...
      if (!typographyPropertyNames.has(lastSegment ?? "")) {
    ...
    if (isTypographyGroup(node as DTCGTokenGroup)) {
      ...
      // Don't recurse into typography groups - they'll be composed separately
      return;
    }
...
export function normalizeDTCGToken(
  ...
  if (!normalizedType) {
    warnings.push(
      `Token at path "${path}" has unsupported type "${tokenType}"`
    );
    return null;
  }
