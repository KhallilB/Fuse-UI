...
import type { DTCGToken, DTCGTokenFile } from "../../types/input-sources"
...
describe("dtcg-normalizers", () => {
	...
	describe("flattenDTCGTokens", () => {
		...
		it("does not compose incomplete typography groups", () => {
			...
			// Since it's not a valid typography group, fontWeight should be included as a regular token
			expect(tokens.size).toBe(1)
			expect(tokens.has("typography.body.fontWeight")).toBe(true)
		})
		...
		it("does not skip standalone typography property tokens at root level", () => {
			const file: DTCGTokenFile = {
				fontSize: {
					$type: "dimension",
					$value: "16px",
			...
			const tokens = flattenDTCGTokens(file)
			// Should include: fontSize (root), fontWeight (root), and typography.body (composed)
			// Should NOT include: typography.body.fontFamily, typography.body.fontSize (skipped as part of typography group)
			expect(tokens.size).toBe(3)
			expect(tokens.has("fontSize")).toBe(true)
			expect(tokens.has("fontWeight")).toBe(true)
			expect(tokens.has("typography.body")).toBe(true)
			expect(tokens.has("typography.body.fontFamily")).toBe(false)
			expect(tokens.has("typography.body.fontSize")).toBe(false)
		})

		it("does not skip typography property tokens in non-typography groups - simple case", () => {
			const file: DTCGTokenFile = {
				spacing: {
					fontSize: {
						$type: "dimension",
			...
			const tokens = flattenDTCGTokens(file)
			expect(tokens.size).toBe(1)
			expect(tokens.has("spacing.fontSize")).toBe(true)
		})

		it("does not skip typography property tokens in non-typography groups", () => {
			const file: DTCGTokenFile = {
				spacing: {
					fontSize: {
						$type: "dimension",
			...
			const tokens = flattenDTCGTokens(file)
			// Should include: spacing.fontSize (not in typography group), typography.body (composed)
			// Should NOT include: typography.body.fontSize (skipped as part of typography group)
			expect(tokens.size).toBe(2)
			expect(tokens.has("spacing.fontSize")).toBe(true)
			expect(tokens.has("typography.body")).toBe(true)
			expect(tokens.has("typography.body.fontSize")).toBe(false)
	...
	describe("normalizeDTCGToken", () => {
		...
		it("returns null and adds warning for missing $type", () => {
			...
			expect(result).toBeNull()
			expect(warnings).toContain('Token at path "color.primary" missing $type')
		})
	...
	describe("integration: full token file normalization", () => {
		it("normalizes a complete DTCG token file", () => {
			...
			for (const [path, token] of flattened.entries()) {
				const normalized = normalizeDTCGToken(path, token, flattened, warnings)
				if (normalized) {
