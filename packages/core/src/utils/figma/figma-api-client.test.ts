...
describe("FigmaApiClient", () => {
  ...
  describe("constructor", () => {
    it("throws if API key is missing", () => {
      expect(
        () => new FigmaApiClient({ apiKey: "", fileKey: mockFileKey })
      ).toThrow("Figma API key (Personal Access Token) is required");
    });

    it("throws if file key is missing", () => {
      expect(
        () => new FigmaApiClient({ apiKey: mockApiKey, fileKey: "" })
  ...
  describe("fetchVariables", () => {
    ...
    it("handles rate limiting", async () => {
      ...
      await expect(client.fetchVariables()).rejects.toThrow(
        "Rate limit exceeded"
      );
    });
  ...
  describe("fetchVariableCollections", () => {
    it("fetches collections successfully", async () => {
      const mockResponse = {
        meta: {
          variableCollections: { "col-1": { id: "col-1", name: "test" } },
        },
      };
      (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
        ok: true,
